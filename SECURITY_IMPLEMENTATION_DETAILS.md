...
`UserOutCountFreezeService::processOutCountAndFreeze` 実行時、`users.sanctions_out_count_snapshot`（前回この処理で保存したアウト合計）と **いまの** `calculateOutCount()` を比較する。**#15 利用に関する警告** と **#16 一時凍結** は、**アウト数がスナップショットより増えたときだけ** 新たに発動する（減算・通報の承認取り消し・時効・減分の手動修正などで下がった場合や横這いの場合は、新規に警告・一時凍結を開始しない）。**#17 永久凍結** は従来どおり現アウトが 4 以上なら適用し、増減フラグは見ない。処理の末尾でスナップショットは常に現在のアウト合計に更新される。スナップショットが `NULL` のユーザーは初回のみ「増加」とみなす。
...
凍結中のユーザーがトップから送信した **異議申立て** に対し、管理者が「凍結・異議申立て一覧」で承認または拒否したときに送られる。タイトルは **承認・拒否とも同一**。
...
```text
ご提出いただいた凍結に関する異議申し立てを確認の上、承認しました。
...
**#21（異議申立て・拒否）** — `allows_reply` = false
