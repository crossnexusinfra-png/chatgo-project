...
| # | 概要 | トリガー | 宛先 | タイトル | 備考 |
|---|------|----------|------|----------|------|
| 20 | 異議申立て・承認（アウト減算） | `AdminController::approveFreezeAppeal` → `sendFreezeAppealApprovalMessage` | 申立ユーザー | 異議申し立ての対応について | `allows_reply` = false。減算後、`UserOutCountFreezeService::processOutCountAndFreeze(..., true)` により **一時凍結は再適用されず** `frozen_until` / `freeze_period_started_at` を解消し、`freeze_count` を **1 減算**（下限 0、アウト &lt; 1 のときは 0 にリセット）。本文に減算額は含めない。 |
