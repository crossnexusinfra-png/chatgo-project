...
## 4. 同一オリジンAPI（通報取得など）が 403 / チャレンジページになる場合

ブラウザからの `fetch` で `/api/reports/existing` などを呼ぶと、Cloudflare がボットと判断して「Just a moment...」チャレンジや 403 を返すことがあります。

**対処**: 同一サイトからの API リクエストにチャレンジをかけないようにする。

**設定場所**: Security → WAF → Custom rules（または Configuration Rules）

### 手順例（Custom rule）
1. 「Create rule」で新規ルールを作成
2. **Rule name**: 例）Skip challenge for same-origin API
3. **Expression (when)**:
   - `(http.request.uri.path contains "/api/" and http.referer contains "https://chatgo.thecrossnexus.com")`  
   - または `(http.request.uri.path eq "/api/reports/existing" and any(http.request.headers["x-requested-with"][*] eq "XMLHttpRequest"))`
4. **Action**: Skip → 該当するセキュリティ機能（Super Bot Fight Mode / Challenge など）をスキップ
5. 保存してデプロイ

これで同一オリジンからの通報取得リクエストがオリジン（Laravel）に届き、既存通報内容が表示されるようになります。
