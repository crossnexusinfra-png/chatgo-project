...
@section('title')
    {{ \App\Services\LanguageService::trans('search_results_title', $lang ?? \App\Services\LanguageService::getCurrentLanguage()) }}
@endsection
...
<div class="main-container">
    ...
    <main class="main-content">
            ...
            <div class="tag-results">
                <p class="tag-info">
                    {{ str_replace('{query}', $query ?? '', \App\Services\LanguageService::trans('search_results_for', $lang)) }}: {{ isset($totalCount) ? $totalCount : (isset($threads) && $threads ? $threads->count() : 0) }}{{ \App\Services\LanguageService::trans('search_results_count', $lang) }}
                    <a href="{{ route('threads.index') }}" class="clear-tag">{{ \App\Services\LanguageService::trans('clear_search_link', $lang) }}</a>
                </p>
            </div>
            
            <!-- フィルタリング・ソート機能 -->
            <div class="filter-section">
                <form method="GET" action="{{ route('threads.search') }}" class="filter-form">
            ...
            <section class="search-threads">
                ...
                <div class="posts-grid">
                    ...
                    @else
                        <div class="no-posts">
                            @if($query && mb_strlen(str_replace(' ', '', str_replace('-', '', $query))) < 2)
                                <p>{{ \App\Services\LanguageService::trans('search_min_length_error', $lang) }}</p>
                                <p class="search-hint">{{ \App\Services\LanguageService::trans('search_hint_title', $lang) }}</p>
                                <ul class="search-hint-list">
                                    <li>{{ \App\Services\LanguageService::trans('search_hint_and', $lang) }}</li>
                                    <li>{{ \App\Services\LanguageService::trans('search_hint_exclude', $lang) }}</li>
                                    <li>{{ \App\Services\LanguageService::trans('search_hint_example', $lang) }}</li>
                                </ul>
                            @else
                                <p>{{ str_replace('{query}', $query ?? '', \App\Services\LanguageService::trans('no_search_results', $lang)) }}</p>
                            @endif
                        </div>
                    @endif
    ...
    <div
        id="thread-search-config"
        data-query="{{ $query ?? '' }}"
        data-sort-by="{{ $sortBy ?? 'latest' }}"
        data-period="{{ $period ?? '' }}"
        ...
        hidden
    ></div>
    <script src="{{ asset('js/thread-search.js') }}" nonce="{{ $csp_nonce ?? '' }}"></script>
@endsection
