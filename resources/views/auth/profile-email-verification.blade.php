...
<head>
    ...
    <title>{{ \App\Services\LanguageService::trans('email_verification_title', $lang) }} - Chatgo</title>
...
<body>
    <div class="auth-container">
        <div class="auth-card verification-card">
            ...
            <form method="POST" action="{{ route('profile.email-verify') }}" class="auth-form">
                @csrf
            ...
            <div class="verification-actions">
                <form method="POST" action="{{ route('profile.email-resend') }}" class="resend-form">
                    @csrf
    ...
    <div
        id="auth-profile-email-verification-config"
        data-time-left="600"
        ...
        hidden
    ></div>
    <script src="{{ asset('js/auth-profile-email-verification.js') }}" nonce="{{ $csp_nonce ?? '' }}"></script>
</body>
</html>
