...
<head>
    ...
    <title>{{ \App\Services\LanguageService::trans('sms_verification_title', $lang) }} - Chatgo</title>
...
<body>
    <div class="auth-container">
        <div class="auth-card verification-card">
            ...
            <form method="POST" action="{{ route('register.sms-verify') }}" class="auth-form">
                @csrf
            ...
            <div class="verification-actions">
                <form method="POST" action="{{ route('register.sms-resend') }}" class="resend-form">
                    @csrf
            ...
            <div class="auth-footer">
                <a href="{{ route('register') }}" class="back-link">← {{ \App\Services\LanguageService::trans('back_to_register', $lang) }}</a>
            </div>
        </div>
    </div>
    
    <script nonce="{{ $csp_nonce ?? '' }}">
        window.authSmsVerificationConfig = {
            timeLeft: 300,
            resendTimeLeft: 60,
            translations: {
    ...
    <script src="{{ asset('js/auth-sms-verification.js') }}" nonce="{{ $csp_nonce ?? '' }}"></script>
</body>
</html>
